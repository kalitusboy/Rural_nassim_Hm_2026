...
class _AdvancedStatsScreenState extends State<AdvancedStatsScreen> {
  late List<Beneficiary> _completed;
  late List<Beneficiary> _all;

  // إحصائيات حسب البرنامج
  ...
  // إحصائيات الصور حسب الحالة
  late Map<String, Map<String, int>> _imageStats;
  ...
  void _calculateImageStats() {
    ...
    for (var b in _completed) {
      ...
      if (b.imagePath != null && b.imagePath!.isNotEmpty) {
        try {
          hasImage = File(b.imagePath!).existsSync();
        } catch (_) {
          hasImage = false;
        }
      }
  ...
  Future<void> _exportToCSV() async {
    try {
      ...
      // الإحصائيات القديمة (حسب الحالة)
      csvContent.writeln('\n=== إحصائيات الخدمات حسب الحالة (بدون تفصيل برنامج) ===');
      ...
      csvContent.writeln('الحالة,عدد المكتملين,لديهم صورة,ليس لديهم صورة');
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // ========== الإحصائيات القديمة (حسب الحالة الكلية) ==========
            const Text('📈 إحصائيات الخدمات حسب الحالة (بدون تفصيل برنامج)', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
            const SizedBox(height: 8),
            Container(
              decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16), boxShadow: [BoxShadow(color: Colors.black12, blurRadius: 8)]),
              child: Padding(
            ...
            // ========== إحصائيات الصور ==========
            const Text('📸 إحصائيات الصور للمكتملين حسب حالة البناية', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
            const SizedBox(height: 8),
            Container(
              decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16), boxShadow: [BoxShadow(color: Colors.black12, blurRadius: 8)]),
              child: Padding(
                ...
                child: SingleChildScrollView(
                  ...
                  child: DataTable(
                    ...
                    columns: const [
                      DataColumn(label: Text('الحالة')),
                      DataColumn(label: Text('عدد المكتملين')),
